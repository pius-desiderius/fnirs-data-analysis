...
for filename in subfolders:
        ...
        def get_max_time(cond_left, time_limits=[int((2-TMIN)*SFREQ), int((11-TMIN)*SFREQ)], median=False):
                if median:
                ...
                peak_timestamp_left = int(np.argmax(mean_left_cond_limited))
        ...
        final_timestamp = get_max_time(f'SA_LEFT')
        rel_final_timestamp = get_max_time(f'rel_SA_LEFT', median=True)
        LOWER_TIME, UPPER_TIME = rel_final_timestamp-1*SFREQ, rel_final_timestamp+1*SFREQ
        peak_time = final_timestamp/SFREQ + 2
        rel_peak_time = rel_final_timestamp/SFREQ + 2
        ...
        for coord in target_chans:
                axes[0].plot(*coord, marker='o', color='#717171', ms=10)
        ...
        a = mne.viz.plot_topomap(data=rest_in_peak_timestamp,
                                ...
                                show=False)
        for coord in target_chans:
                axes[1].plot(*coord, marker='o', color='#717171', ms=10)
        ...
        for coord in target_chans:
                ax.plot(*coord, marker='o', color='#717171', ms=10)
